...
export default function AdminDashboard() {
  ...
  if (!token)
    return (
      <div className="min-h-screen bg-main text-white p-10">Loading...</div>
    );

  return (
    <div className="min-h-screen bg-main text-white p-10">
        ...
        <div className="space-y-8">
          <section>
            <h2 className="text-xl font-semibold mb-4 border-b border-white/10 pb-2">
              تنظیمات بخش‌های سایت
            </h2>
            <div className="grid gap-4">
              {settings.map((setting) => (
                <div
                  key={setting.ID}
                  className="flex justify-between items-center p-4 border border-white/5 bg-white/5 rounded-xl"
                >
                  ...
                  <button
                    onClick={() =>
                      toggleSetting(setting.SectionName, !setting.IsVisible)
                    }
              ...
              {settings.length === 0 && (
                <p className="text-neutral-500">
                  هیچ تنظیمی یافت نشد (باید در دیتابیس ایجاد شوند)
                </p>
              )}
            </div>
          </section>
